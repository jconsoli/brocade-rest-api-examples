...
def pseudo_main(user_id, pw, ip, sec, vd, log, nl):
    ...
    :type vd: bool
    :param log: Log file prefix. Typically, a folder name followed by '/'
    :type log: str
    :param nl: If True, do not create a log file
    :type nl: bool
    :return: Exit code
    :rtype: int
    """
    ...
    ec, fid, save_flag = 0, _DEBUG_FID, False

    if vd:
        brcdapi_rest.verbose_debug(True)
    brcdapi_log.open_log(folder=log, suppress=False, version_d=brcdapi_util.get_import_modules(), no_log=nl)
    ...
    try:
        # Create aliases
        if ec == 0 and _DEBUG_CREATE_ALIASES:
            save_flag = True
        ...
        # Create zones
        if ec == 0 and _DEBUG_CREATE_ZONES:
            save_flag = True
        ...
        if ec == 0 and _DEBUG_CREATE_ZONECFGS:
            save_flag = True
        ...
        if ec == 0 and _DEBUG_DELETE_ZONECFGS:
            save_flag = True
        ...
        if ec == 0 and _DEBUG_DELETE_ZONES:
            save_flag = True
        ...
        # Delete aliases
        if ec == 0 and _DEBUG_DELETE_ALIASES:
            save_flag = True
        ...
        if ec == 0 and _DEBUG_MODIFY_ZONE:
            save_flag = True
        ...
        if ec == 0 and _DEBUG_MODIFY_ZONECFG:
            save_flag = True
        ...
        if ec == 0 and _DEBUG_DISABLE_ZONECFG:
        ...
        if ec == 0 and _DEBUG_ENABLE_ZONECFG:
