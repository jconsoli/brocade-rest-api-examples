...
"""
:mod:`port_enable_all.py` - Enable ports in a FID on a chassis.
...
**Description**
    ...
    * Enable a list of FC ports (with comments on how to disable them)
...
def parse_args():
    ...
    if _DEBUG:
        ...
        parser = argparse.ArgumentParser(description='Enable all ports in a logical switch.')
...
def pseudo_main():
    ...
    try:  # I always do a try in code development so that if there is a code bug, I still log out.
        ...
        if brcdapi_auth.is_error(obj):
            ...
            obj = brcdapi_port.enable_port(session, fid, fc_plist, True)  # brcdapi_port.disble_port() to disable
            if brcdapi_auth.is_error(obj):
                brcdapi_log.log('Failed to enable ports\n' + brcdapi_auth.formatted_error_msg(obj), True)
                ec = -1
            else:
